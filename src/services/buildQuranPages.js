...
// ── Index de navigation ──
// Pour chaque sourate : première page
const surahIndex = {};
// Pour chaque juz : première page
const juzIndex = {};
// Pour chaque hizb : première page
const hizbIndex = {};

qaloonData.forEach(aya => {
  const page = parseInt(aya.page);

  // Sourate : première occurrence = première page
  ...
  // Juz : première occurrence = première page
  ...
  // Hizb : première occurrence = première page
  if (aya.hizb && !hizbIndex[aya.hizb]) {
    hizbIndex[aya.hizb] = {
      hizb_number: aya.hizb,
      first_page:  page,
    };
  }
});
...
for (let pageNum = 1; pageNum <= 604; pageNum++) {
  ...
  // Sourates présentes sur cette page
  const surahsMap = {};
  ayahs.forEach(aya => {
    if (!surahsMap[aya.sura_no]) {
      surahsMap[aya.sura_no] = {
        ...
        sura_name_en: aya.sura_name_en,
        type:         SURAH_TYPE[aya.sura_no] || 'meccan',
        // Est-ce que cette sourate COMMENCE sur cette page ?
  ...
  const filePath = path.join(OUTPUT_DIR, `page-${pageNum}.json`);
  fs.writeFileSync(filePath, JSON.stringify(pageJson, null, 2), 'utf8');

  if (pageNum % 100 === 0) console.log(`✅ ${pageNum}/604`);
}

// ── Générer l'index de navigation global ──
const navIndex = {
...
console.log(`\n🎉 604 fichiers générés dans src/data/quran-pages/`);
console.log(`📚 Index de navigation : src/data/quranIndex.json`);
console.log(`   Sourates : ${navIndex.surahs.length}`);
console.log(`   Juz      : ${navIndex.juzs.length}`);
console.log(`   Hizbs    : ${navIndex.hizbs.length}`);
let indexContent = '// AUTO-GENERATED\nconst pages = {\n';
for (let i = 1; i <= 604; i++) {
  indexContent += `  ${i}: require('./quran-pages/page-${i}.json'),\n`;
}
...
console.log('📄 Index statique généré : src/data/quranPagesIndex.js');
// ── Index statique pour mushaf-lines ──
const LINES_DIR = path.join(__dirname, '../data/mushaf-lines');
const availablePages = [];

for (let i = 1; i <= 604; i++) {
  if (fs.existsSync(path.join(LINES_DIR, `page-${i}.json`))) {
    availablePages.push(i);
  }
}

let linesIndexContent = '// AUTO-GENERATED\nconst linesPages = {\n';
availablePages.forEach(i => {
  linesIndexContent += `  ${i}: require('./mushaf-lines/page-${i}.json'),\n`;
});
linesIndexContent += '};\nexport default linesPages;\n';

fs.writeFileSync(
  path.join(__dirname, '../data/mushaflinesIndex.js'),
  linesIndexContent,
  'utf8'
);
console.log(`📄 mushafLinesIndex généré : ${availablePages.length} pages disponibles`);